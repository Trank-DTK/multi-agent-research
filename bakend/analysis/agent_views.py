...
class AnalysisAgentChatView(APIView):
    ...
    def post(self, request, dataset_id):
        ...
        try:
            ...
            response = build_llm(request.user).chat([{'role': 'system', 'content': '你是科研数据分析助手。依据给定预览和统计摘要回答，不把样本预览当作完整数据，不虚构已运行的检验。需要进一步计算时明确说明。'}, {'role': 'user', 'content': f'数据集：{dataset.name}，{len(df)} 行\n统计摘要：\n{summary}\n前10行：\n{preview}\n问题：{message}'}])
